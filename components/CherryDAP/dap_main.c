...
volatile uint32_t g_chry_dap_last_activity_tick = 0;
...
void chry_dap_handle(void)
{
    ...
    while (USB_RequestCountI != USB_RequestCountO) {
        ...
        g_chry_dap_last_activity_tick = (uint32_t) xTaskGetTickCount();
...
void chry_dap_usb2uart_handle(void)
{
    ...
    */
    ...
    /* uartrx to usb tx */
    if (usbtx_idle_flag) {
        uint32_t used = chry_ringbuffer_get_used(&g_uartrx);
        if (used > 0) {
            // Manually copy data from ringbuffer to safe usb_tmpbuffer to avoid DMA issues
            // Reuse usb_tmpbuffer as it is NOCACHE and available
            uint32_t len = (used > DAP_PACKET_SIZE) ? DAP_PACKET_SIZE : used;
            chry_ringbuffer_read(&g_uartrx, usb_tmpbuffer, len);
            usbtx_idle_flag = 0;
            usbd_ep_start_write(0, CDC_IN_EP, usb_tmpbuffer, len);
        }
    }
...
//static portMUX_TYPE log_spinlock1 = portMUX_INITIALIZER_UNLOCKED;

#define BUF_SIZE_CDC (64) //A bug: 128 or 256 does not work. Not fixed yet
int usb_cdc_log_vprintf(const char *format, va_list args)
{
    //portENTER_CRITICAL(&log_spinlock1);
    static USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t buf[BUF_SIZE_CDC];
    // Format the log message
    int len = vsnprintf((char*)buf, BUF_SIZE_CDC, format, args);
    //len = snprintf((char*)buf, BUF_SIZE_CDC, "len=%d              end\n", len);
    int len_max = BUF_SIZE_CDC;
    // Truncate chars to len_max
    if (len >= len_max){
        len = len_max;
    }
    ...
    if (len > 0) {
        ...
        /*
        if (chry_ringbuffer_get_free(&g_uartrx) >= len) {
            chry_ringbuffer_write(&g_uartrx, (uint8_t *)buffer, len);
        }*/
        usbtx_idle_flag = 0;
        usbd_ep_start_write(0, CDC_IN_EP, buf, len);
    }
    //portEXIT_CRITICAL(&log_spinlock1);
    return len;
}
