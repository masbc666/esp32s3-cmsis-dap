...
static uint8_t* dtmp = NULL;

void chry_dap_usb2uart_uart_config_callback(struct cdc_line_coding *line_coding);

void uart_event_task(void *pvParameters)
{
    //ESP_LOGD(TAG, "uart_event_task start", EX_UART_NUM);
   // for (;;)
    //{
        uart_event_t event;
        ...
        if (buffered_size > 0)
        {
            ...
            // ESP_LOGD(TAG, "[UART DATA]: %d: '%s'", buffered_size, dtmp);
     ...
     //   vTaskDelay(1);
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (TickType_t)1)) {
            bzero(dtmp, RD_BUF_SIZE);
            // ESP_LOGD(TAG, "uart[%d] event: %d", EX_UART_NUM, event.type);
            switch (event.type) {
            ...
            case UART_DATA:
                ...
                // ESP_LOGD(TAG, "[UART DATA]: %d: %s", event.size, dtmp);
    ...
    //}
#if 0
    ESP_LOGD(TAG, "uart_event_task stop", EX_UART_NUM);
    free(dtmp);
    dtmp = NULL;
    vTaskDelete(NULL);
#endif
...
void chry_dap_usb2uart_uart_config_callback(struct cdc_line_coding *line_coding)
{
    ...
    uart_set_pin(UART_PORT_NUM, DAP_UART_TX, DAP_UART_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
